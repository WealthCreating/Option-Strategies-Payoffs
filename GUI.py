...
class MainWindow(QMainWindow):                  
    ...
    def iniUI(self):
        ...
        fileOpBear.triggered.connect(self.getInt)
    ...
    def optimise_bull_call_spread(self, target):
        nifty = int(round(st,-2))
        range_of_analysis = np.arange(nifty-100,target,50)
        ...
        choice = ['Max Profit','Min Risk']
        x, ok = QInputDialog.getItem(self,'Optimisation','Optimise based on :',choice,0,False)
        if ok:
            selected_choice = x
        if selected_choice == 'Max Profit':
            max_profit = max(bcsdf['Profit'])
            p = (bcsdf['Profit'] == max_profit).nonzero()
            ...
            QMessageBox.information(self, 'Optimized based on Max Profit', "As per the optimisation :\n Buy {} Strike Call Option and Sell {} Strike Call Option \n This spread will have the Maximum Profit Potential for your Target of : {}".format(long_call_SP,target,target), QMessageBox.Ok, QMessageBox.Ok)
            self.show_charts(bdf)
        else:
            min_risk = min(bcsdf['Risk'])
            p = (bcsdf['Risk'] == min_risk).nonzero()
            long_call_SP = bcsdf.loc[int(p[0]),'Long_Call_Strike']
            z = (CE['STRIKE_PR'] == long_call_SP).nonzero()
            long_call_premium = CE.loc[int(z[0]),'CLOSE']
            long_call_payoff = call_payoff(range_of_payoff,long_call_SP,long_call_premium)
            ...
            bdf = pd.DataFrame({'Nifty_Spot_Price':range_of_payoff,
                                    ...
                                    'Bull_Call_Spread':bcs_payoff})
            bdf = bdf[['Nifty_Spot_Price','Long_Call_Payoff','Short_Call_Payoff','Bull_Call_Spread']]
            QMessageBox.information(self, 'Optimized based on Min Risk', "As per the optimisation :\n Buy {} Strike Call Option and Sell {} Strike Call Option \n This spread will have the minimum risk for your Target of : {}".format(long_call_SP,target,target), QMessageBox.Ok, QMessageBox.Ok)
            self.show_charts(bdf)
        
    #---------------------------------END OF BULL CALL SPREAD SECTION--------------------------------------------
    ...
    def plot_bull_put_spread(self):
        ...
        self.Table.resizeColumnsToContents()
        self.Table.resizeRowsToContents()
    #---------------------------------END OF BULL PUT SPREAD SECTION---------------------------------------------
    def call_ratio__back_spread(self):
        """
